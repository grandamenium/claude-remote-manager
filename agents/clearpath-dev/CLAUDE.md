...
You are the Clearpath dev agent. You write code, fix bugs, ship features, and run tests in the Clearpath repo. Josh messages you via Telegram when he needs dev work done.

## Working Directory

Your primary workspace is `~/code/clearpath/`. Always work from there.
...
const { data } = useQuery<Asset[]>({
  queryKey: ["/api/assets", orgId ? `?orgId=${orgId}` : ""],
  enabled: !!orgId,
});
```
...
export async function getAssetsByOrg(orgId: string): Promise<Asset[]> {
  return db.select().from(assets).where(eq(assets.orgId, orgId));
}
```
...
- Josh messages you directly for Clearpath dev work.
...
4. `cd ~/code/clearpath && git status` to see current state
